...
The great people at Let's Encrypt provides two tools to test ACME clients, [Pebble](https://github.com/letsencrypt/pebble/) and [Boulder](https://github.com/letsencrypt/boulder).
You can read more about the two tools in their home pages, but here's a quick guide to configure them:

### Pebble

1. Install [Go](https://golang.org/dl/)
2. Open a terminal console and type:
    ```
    ...
    # On Windows
    cd /D "%GOPATH%\src\github.com\letsencrypt\pebble"
    
    go install ./...
    ```
3. Create a new folder (wherever you prefer, let's call it `x`), and copy there these files present in your `GOPATH` folder:
    1. `src/github.com/letsencrypt/pebble/test/config/pebble-config.json`
    2. `src/github.com/letsencrypt/pebble/test/certs/localhost/key.pem`
    3. `src/github.com/letsencrypt/pebble/test/certs/localhost/cert.pem`
4. Edit your copy of `pebble-config.json` and change
    1. the value of the `httpPort`, setting it to the port of your local website
    2. the value of the `certificate` and `privateKey` fields, by removing the `test/certs/localhost` part
5. Enter your `x` directory, and launch:
    ```
    # On Mac/*nix
    "$GOPATH/bin/pebble.exe" -config "$CD/pebble-config.json"
    # On Windows
    "%GOPATH%\bin\pebble.exe" -config "%~dp0pebble-config.json"
    ```
...
3. Open a terminal console and type:
    ```
    # On Mac/*nix
    ...
    # On Windows
    git clone https://github.com/letsencrypt/boulder/ "%GOPATH%\src\github.com\letsencrypt\boulder"
    cd /D "%GOPATH%\src\github.com\letsencrypt\boulder"
    
    docker-compose up
    ```
    You need to know the host IP seen by the docker containers.
    To do that, open a terminal console (while the Boulder is running), and type:
    ```
    docker exec -it boulder_bmysql_1 bash
    apt update && apt install -y iputils-ping
    ping host.docker.internal
    ```
    that should display the host IP. If for example it's `192.168.65.2`, stop the docker containers, and restart them with the following options:
    ```
    docker-compose run --use-aliases -e FAKE_DNS=192.168.65.2 --service-ports boulder ./start.py
    ```
4. Add a new entry in your local hosts file (`/etc/hosts` on Mac/*nix, `C:\Windows\System32\drivers\etc\hosts` on Windows), adding a line like this:
    `127.0.0.1       boulder`
